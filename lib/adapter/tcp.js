...
/**
 * Connection lifecycle event bus. Consumers may subscribe with:
 *   adapter.events.on('connect', function ({ slotIndex }) { ... });
 *   adapter.events.on('disconnect', function ({ slotIndex, error, queueSize }) { ... });
 *   adapter.events.on('reconnectScheduled', function ({ slotIndex, delayMs, attempt }) { ... });
 *   adapter.events.on('queueOverflow', function ({ slotIndex, behavior }) { ... });
 *   adapter.events.on('giveUp', function ({ slotIndex, queueSize }) { ... });
 *
 * Note: this adapter is a module singleton, so all consumers in the same
 * process share the same emitter. tcp-tls inherits this emitter via prototype.
 */
...
adapter._connect = function (slot) {
  ...
  client.once('connect', function () {
    ...
    client.setTimeout(0);
    self._emit('connect', { slotIndex: slot.slotIndex });
    self._drain(slot);
  });
...
adapter._scheduleReconnect = function (slot) {
  ...
  this._emit('reconnectScheduled', {
    slotIndex: slot.slotIndex,
    delayMs: slot.reconnectDelay,
...
adapter._enqueue = function (slot, packet, callback) {
  ...
  if (slot.queue.length >= kaOpts.maxQueueSize) {
    this._emit('queueOverflow', {
      slotIndex: slot.slotIndex,
      behavior: kaOpts.queueFullBehavior
    });
...
adapter._sendPersistent = function (message, callback) {
  ...
  if (slot.state === 'disconnected') {
    // Only kick a fresh connection attempt if there isn't already one
    // scheduled. Cancelling the backoff timer and resetting reconnectAttempts
    // on every new send (the previous behaviour) collapsed exponential
    // backoff to reconnectBaseDelay for any caller that logged steadily
    // during an outage. reconnectAttempts is reset on a successful connect
    // (see _connect) which is the only place that should clear it.
    if (!slot.reconnectTimer) {
      this._connect(slot);
    }
...
/**
 * Safely emits a lifecycle event. Listener errors are caught so a buggy
 * subscriber can never break the connection state machine.
 * @param {String} name
 * @param {Object} payload
 */
adapter._emit = function (name, payload) {
  if (!this.events) {
    return;
  }
  try {
    ...
    // swallow - listener errors must not corrupt adapter state
  }
};

/**
 * Returns a snapshot of the connection pool's state. Safe to call before
 * the pool is initialized (returns initialized:false in that case).
 * @returns {Object}
 */
adapter.getStats = function () {
  if (!this._persistentInitialized) {
    return { initialized: false, poolClosed: false, slots: [] };
  }
  return {
    initialized: true,
    poolClosed: !!this._poolClosed,
    slots: this._pool.map(function (slot) {
      return {
        index: slot.slotIndex,
        state: slot.state,
        queueSize: slot.queue.length,
        reconnectAttempts: slot.reconnectAttempts,
        reconnectScheduled: !!slot.reconnectTimer,
        inflightCount: slot.currentBatch ? slot.currentBatch.length : 0
      };
    })
  };
};
